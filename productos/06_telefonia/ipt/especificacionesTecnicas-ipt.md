# ESPECIFICACIONES TÉCNICAS Y ALCANCE DE SERVICIO - IP TELEPHONY – SIN ÚLTIMA MILLA (IPT)

## 1. DESCRIPCIÓN GENERAL

El servicio de IP telephony (IPT) le brindará la posibilidad de conectar la central privada de su empresa con el Soft Swich de Metrotel a través de internet. El servicio podrá ser adquirido en dos modalidades:
+ Con abono mensual (IPT – Troncal IP)
+ Prepago (IPT – Troncal IP prepago)

Con el servicio de IPT de Metrotel, su empresa estará conectada con la red PSTN convencional y la numeración asignada será accesible desde toda la red pública global. El servicio se presenta en paquetes de diferentes configuraciones, cantidad de números y canales. Con una mínima inversión tendrá variedad de planes según su consumo, máxima calidad, rápida instalación y flexibilidad. Entre los principales beneficios se destacan:
+ Reducción de costos operativos mejorando sus abonos y tarifas. Con el fraccionamiento de las llamadas por minuto se puede ahorrar desde un 30 % en el costo de sus comunicaciones.
+ Funcionamiento en una red de última tecnología IP obteniendo privacidad y seguridad en sus comunicaciones.
+ Posibilidad de recibir múltiples llamadas simultáneas al mismo número.

## 2. ALCANCE Y ESPECIFICACIÓN TÉCNICA

El servicio IPT, fue diseñado de un modo totalmente dinámico y flexible, de modo de que pueda optar por la configuración que mejor se ajuste a las necesidades de su negocio. 
En ese sentido, el servicio posee múltiples versiones de cantidad de canales, todas ellas con una única cuenta/cliente SIP, y opcionalmente se puede escoger la cantidad de números telefónicos, el número de cabecera, precompras para tráfico local, LDN y LDI que necesite (solo servicios con Abono), o algunas opciones de bloqueos y listas que pueden contratarse directamente por el
módulo de autogestión web que se entrega al cliente.

El servicio soporta los códec ILBC, G726, GSM (orientado a Asterisk) y G729 en modo forzado (orientado a Vendors). Dado los codecs mencionados las comunicaciones de Fax, Modem o Postnet no serán soportadas.
...
<sup>(2*)</sup> *Opcionales no disponibles para la modalidad prepaga (IPT – Troncal IP prepago)*
...
**Parámetros de Calidad en la conexión a Internet:**

Para garantizar la calidad del servicio IPT provisto por Metrotel, la conexión a internet contratada a terceros deberá cumplir con los siguientes parámetros mínimos de calidad de servicio:
1. Retardo inferior a 100 ms
2. Jitter (variación del retardo) inferior a 5 ms
3. Perdida de paquetes inferior a 1%
4. Ancho de banda disponible por cada canal de voz (simultaneo), unos 45 kbps.
...
Metrotel garantizará el correcto funcionamiento de la plataforma de telefonía que se utilizara para interconexión del servicio con la red pública de telefonía (PSTN).

La responsabilidad de Metrotel en su calidad de prestador del servicio de telecomunicaciones y de conformidad con la normativa vigente, se extiende hasta la prestación del servicio y no dentro de los límites internos de las redes informáticas y/o de telecomunicaciones del Cliente.
...
El Cliente no podrá alegar incrementos súbitos de tráfico y/o acceso a destinos inusuales comparado con el histórico de tráfi co debidos a un ataque informático, ni cualquier otro argumento como excusa para no efectuar el pago de ese tráfico incremental y/o inusual.
...
Metrotel ofrece dos tipos de modalidades en la autorización del troncal SIP. El primero es por IP fija y el Segundo por Registración:
+ **IP Fija:** se valida el usuario por Dicha IP y por el Usuario SIP descripto en el Encabezado del “FROM” en el paquete SIP.
+ **Registración:** Esta permite validarse por el método REGISTER con las credenciales provistas por Metrotel, y puede también restringirse a una o varas IP’s a solicitud del cliente. Se establece un límite de 2 registros en simultáneo a los efectos de ofrecer redundancia y/o Balanceo para el cliente. El cliente puede generar llamadas solo si existe activa la Registración y en cada Una de las llamadas Se validara con las Credenciales provistas.
...
Metrotel asegurará la concurrencia del total de llamadas según el límite ofrecido y contratado, manteniendo un ritmo de establecimiento de llamadas por segundo de acuerdo al párrafo siguiente:

Hasta 50 canales contratados: 2 CPS - Más de 50 canales contratados: 4 CPS

Metrotel no será responsable por el incorrecto funcionamiento del servicio o por el descarte de llamadas en caso que el equipamiento utilizado por el Cliente intente establecer mayor cantidad de llamadas en un segundo. En las llamadas donde el destino genere un corte de la llamada produciéndose una suspensión de la misma, Metrotel no realizará una señalización especial y enviara el audio que recibe directamente desde el destino.
