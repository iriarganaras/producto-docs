# ESPECIFICACIONES TÉCNICAS Y ALCANCE DE SERVICIO - SERVIDOR VIRTUAL BACKUP

## 1. DESCRIPCIÓN GENERAL

El producto Servidor Virtual Backup ofrece al cliente los recursos necesarios para ejecutar sus aplicaciones de negocio en instalaciones del Data Center de Metrotel.
...
De esta manera, con los recursos virtualizados por este método, Metrotel puede instanciar una máquina virtual en la que se instalará el sistema operativo y las aplicaciones acordes a su necesidad.

## 2. VENTAJAS DEL PRODUCTO
+ Infraestructura escalable, rápido aprovisionamiento bajo demanda, ideal para procesos críticos.
+ Disponible para la creación de servidores virtuales Linux y en caso de Windows, previa contratación.
+ El servicio podrá ser contratado en múltiples opciones de recursos de acuerdo con las necesidades de cada cliente.
...
Los Servidores Virtuales de Metrotel se encuentran disponibles en varias versiones: Low, Medium, y High.

Las variantes están dadas por distintas combinaciones de:
+ Procesamiento
+ Memoria
+ Espacio en Disco
...
El procesador es un parámetro que se fija con cada variante de producto y puede aumentarse a demanda del cliente, dentro de ciertos límites, solo debe reiniciarse. Al tratarse de un servicio virtual, el procesador también lo es, lo que significa que se trata de una porción de procesador que el sistema de virtualización asigna a cada máquina virtual. Este cambio se realiza con una breve interrupción programada del servicio.

**Memoria RAM**

Al igual que en el caso del procesador, la memoria RAM es un parámetro ya definido en cada versión del servicio y puede aumentarse a demanda del cliente, dentro de ciertos límites, sin necesidad de interrupción del servicio, solo debe reiniciarse. Este cambio se realiza con una breve interrupción programada del servicio.
...
La plataforma de virtualización se encuentra conectada directamente al backbone de Internet y datos de Metrotel, lo que proporciona la máxima velocidad, capacidad y disponibilidad de la conexión.

También puede optar por conexión dedicada permitiéndole elegir entre las diversas alternativas que Metrotel posee:
+ Acceso a Internet compartido: se provee solo el acceso con 1 IP pública por servicio con hasta 10Mb máximo de ancho de banda (Tipo best-effort), pudiendo Metrotel compartir el tráfico entre otros servicios. Metrotel no aconseja usarlo en más de 1 Servidor Virtual, en el caso que necesite más, aconsejamos que contrate el opcional de FW Virtual, el cual le permitiría centralizar en 1 unico lugar la IP pública.
  
También puede optar por conexión dedicada permitiéndole elegir entre las diversas alternativas que Metrotel posee:
+ Acceso a Internet dedicado: Mediante esta alternativa, se provee un acceso a Internet dedicado con la cantidad de IPs públicas que contrate y SLA.
...
Metrotel a su criterio podrá optar por solicitarle al cliente el cambio de servicio de datos que contrató, en caso de monitorear un consumo excesivo en dicho tráfico o un uso indebido del mismo. Además, Metrotel podrá optar por cortar todo el tráfico del servicio de datos contratado, hasta tanto el cliente solucione los problemas que causa dicho tráfico excesivo y que perjudica el buen funcionamiento de la plataforma. El cliente acepta que dicho corte del servicio no contará para el cálculo de disponibilidad del servicio y libera de responsabilidad a Metrotel de todo reclamo y/o cualquier otra pérdida que pudiera sufrir el cliente como consecuencia directa o indirecta.
...
De manera predeterminada, e incluido en el servicio estándar del producto Datacenter Virtual de Metrotel, se tiene parametrizado quince (15) puntos de restauración para la política de resguardo de información, correspondientes a un ciclo de 15 días consecutivos de respaldos, realizándose uno completo (“full backup”) la primera vez que se activa la tarea de resguardo (“job de backup”). El respaldo completo, luego, se repite cada 7 días, mientras que el resto de los días del ciclo se realizan resguardos incrementales.

En caso de requerirse mayor tiempo de retención sobre los datos resguardados, antes de utilizar el servicio se deberá informar a Metrotel para evaluar la factibilidad técnica. El cambio de la retención estándar en la política de resguardo tendrá un costo adicional, como mínimo dependiendo del tiempo de retención que se requiera.

De manera estándar, el producto de Metrotel incluye dos (2) restauraciones de información al mes, en caso de ser necesario mayor cantidad puede aplicarse cargos adicionales.
...
A continuación, se detallan los opcionales que podrán ser contratados adicionalmente a los Servidores Virtuales de Metrotel.
...
+ Para los casos de conectividad a través de Internet dedicado, TLS o RPV, dichos servicios se contratan adicionalmente al servicio de servidor virtual, de acuerdo con la necesidad específica del cliente, previo análisis de factibilidad técnica por parte de Metrotel.
...
+ El Sistema Operativo tiene que ser elegido entre aquellos que figuran a continuación:
    + Linux Debian (64bits)
    + Linux CentOS (64bits)
    + Windows Server Standard (64bits)
+ SQL Server Estándar (el mínimo recomendado de la VM es 2Vpu, 4Gb RAM y 150Gb de disco)
...
## 4. CARACTERÍSTICAS DEL LICENCIAMIENTO
...
Si Metrotel no cuenta con el licenciamiento de la versión que el cliente solicite, y en caso de requerirse el traslado de uso de licencias del cliente, este deberá validarse de acuerdo con el contrato y reglamentación de Microsoft.
...
Las versiones de sistema Operativo Windows deben ser las vigentes según Microsoft. En el siguiente Link, puede consultar la vigencia por versión: https://support.microsoft.com/esbo/lifecycle/search/1163

Asimismo, las versiones a utilizarse deben ser las soportadas por VMWare. En el siguiente Link puede consultar cuales son: https://www.vmware.com/resources/compatibility/search.php?deviceCategory=software&details=1&partner=110&releases=429&productNames=15&page=1&display_interval=10&sortColumn=Partner&sortOrder=Asc&testConfig=16
...
**Aprendizaje.** El Cliente proporcionará formación anual sobre el cumplimiento de las Leyes Anticorrupción a sus empleados.
...
## 6. RESPONSABILIDADES POR PARTE DEL CLIENTE
El cliente es responsable por la seguridad de su servidor virtual.
El cliente podrá acceder a el servidor contratado en forma remota dependiendo del Sistema Operativo, Remote Desktop o SSH, por medio de una conexión a Internet.
...
Metrotel no se hace responsable por la integridad, inconsistencia y/o pérdida de los datos que puedan ocasionar las fallas que involucren a los componentes del equipo adquiridos (hardware/software) ni errores humanos de terceros ajenos a Metrotel.

El servicio no incluye firewall, salvo que el cliente los contrate adicionalmente.

Metrotel no efectuará la gestión del Servidor, entendiéndose por gestión a todas aquellas tareas derivadas de la propia operación y control de funcionamiento del mismo como ser administración del sistema operativo, aplicación de parches correctivos y/o de seguridad, instalación de aplicaciones, resolución de problemas sobre aplicaciones instaladas en el servidor virtual, análisis de performance sobre aplicaciones instaladas en el servidor virtual, monitoreo de aplicaciones ni servidores virtuales, quedando dichas tareas bajo estricta responsabilidad del cliente. En caso de requerirse dicho servicio deberá ser contratado adicionalmente.
